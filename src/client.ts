...
/** PeasyImage API client. Zero dependencies — uses native fetch. */
export class PeasyImage {
  private baseUrl: string;

  constructor(baseUrl = "https://peasyimage.com") {
  ...
  private async get<T>(path: string, params?: Record<string, string | number | undefined> | object): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`PeasyImage API error: HTTP ${res.status} — ${body}`);
    }
